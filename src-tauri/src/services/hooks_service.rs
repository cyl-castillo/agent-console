...
impl HooksRuntime {
    pub fn new() -> AppResult<Self> {
        ...
        let script_path = ensure_hook_script(&cache)?;
        Ok(Self { session_dir, script_path, watcher_started: Mutex::new(false) })
    }
    ...
    pub fn start_watcher(&self, app: AppHandle) {
        ...
        *started = true;
        ...
        thread::spawn(move || watcher_loop(app, dir));
    }

    pub fn status(&self) -> HooksStatus {
        let settings_path = settings_path();
        let installed = is_hook_installed(&settings_path, &self.script_path).unwrap_or(false);
        HooksStatus {
    ...
    /// Merge our UserPromptSubmit hook into ~/.claude/settings.json.
    pub fn install(&self) -> AppResult<HooksStatus> {
        ...
        let entry = json!({
            ...
            "hooks": [{
                "type": "command",
                "command": self.script_path.to_string_lossy(),
            }]
        });

        let arr = hooks.get_mut("UserPromptSubmit")
            .and_then(|v| v.as_array_mut())
            .cloned()
            .unwrap_or_default();
        let already = arr.iter().any(|e| has_command_path(e, &self.script_path));
        let mut new_arr = arr;
        if !already { new_arr.push(entry); }
        hooks.as_object_mut().unwrap()
            .insert("UserPromptSubmit".to_string(), Value::Array(new_arr));
    ...
    /// Remove our hook entry from settings.json. Other hooks/settings untouched.
    pub fn uninstall(&self) -> AppResult<HooksStatus> {
        ...
        if let Some(hooks) = settings.get_mut("hooks").and_then(|v| v.as_object_mut()) {
            if let Some(arr) = hooks.get_mut("UserPromptSubmit").and_then(|v| v.as_array_mut()) {
                arr.retain(|e| !has_command_path(e, &self.script_path));
            }
        }
        fs::write(&settings_path, serde_json::to_string_pretty(&settings).unwrap())?;
        Ok(self.status())
...
fn ensure_hook_script(runtime_dir: &Path) -> AppResult<PathBuf> {
    let path = runtime_dir.join("userprompt-hook.cjs");
    fs::write(&path, USERPROMPT_HOOK)?;
    #[cfg(unix)]
    {
...
fn is_hook_installed(settings_path: &Path, script_path: &Path) -> AppResult<bool> {
    if !settings_path.exists() { return Ok(false); }
    let txt = fs::read_to_string(settings_path)?;
    let v: Value = serde_json::from_str(&txt).unwrap_or(json!({}));
    let Some(arr) = v.pointer("/hooks/UserPromptSubmit").and_then(|v| v.as_array()) else {
        return Ok(false);
    };
