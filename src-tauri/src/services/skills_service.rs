...
fn parse_frontmatter(path: &Path) -> (Option<String>, Vec<String>) {
    let Ok(content) = fs::read_to_string(path) else { return (None, Vec::new()) };
    if !content.starts_with("---") { return (None, Vec::new()); }
    let after = &content[3..];
    ...
    for line in fm.lines() {
        ...
        if let Some(rest) = l.strip_prefix("description:") {
            ...
            allowed_tools = v.split(',').map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect();
        }
    }
...
pub fn read_md(path: &Path) -> AppResult<String> {
    let target = if path.is_dir() { path.join("SKILL.md") } else { path.to_path_buf() };
